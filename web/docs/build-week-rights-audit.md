...
| `assets/dict/オノマトペ.csv` | 現コードから参照なし。第3列のみ利用可とユーザー確認済み | 第3列先頭100件を新教材の語彙入力に使い、CSV自体はGit追跡から除外 |
...
1. ユーザー指定CSVの第3列だけを読み、先頭100件を順番どおり採用する。
...
- 100カード × 5曲のスコア生成と、各対応フレーズへの正解語モーラ割当をテストした。
