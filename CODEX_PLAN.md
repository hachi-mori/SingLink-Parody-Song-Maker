...
最新の `origin/openai-build-week` を基点に第三者教材を自作100件へ置換した状態から、ユーザーが利用を許可したCSV第3列の全335語へ自作教材を拡張する。C++ / Siv3D版は変更しない。
...
1. `web/assets/dict/cards_text_data.json`: 作業開始時は第三者教材由来の100カード、163例文、163意味を含んでいた教材本体。現在は自作335件へ置換済み。
2. `web/assets/dict/cards_singing_readings.json`: 作業開始時は第三者教材由来の例文から生成された100件の歌唱用読み。現在は自作335件へ置換済み。
...
- 採用範囲: CSV第3列の全335件を元の順番どおり採用する。先頭100件は動作確認済みの内容を維持し、101〜335件の235件を追加する。
...
1. ユーザー提供語彙を使った自作335件を、クイズ・表示・意味説明の単一ソースとして格納する。
2. 同じ335件に対応する歌唱用読みを格納し、キー欠落・重複・件数不一致をテストで失敗させる。
...
8. 5曲 × 335カードのスコア生成検証と、1プレイ3問のランダム出題を維持する。

## 親エージェントが定める完了条件

- 提出対象の追跡ファイルとビルド成果物に、現在の第三者教材由来のカード本文・意味・派生読みが残っていない。
- 自作335件がすべて一意で、例文に正解語を含み、意味と歌唱用読みが欠落していない。
- 採用語が許可済みCSVの第3列全335件と順序・集合とも一致し、第1・第2列を教材本文へ流用していない。
- 動作確認済みの先頭100件の例文・意味・読みが変更されていない。
...
- Terra子エージェント: 追加235件の教材・読み生成、335件対応テスト更新、Webテスト実行。
